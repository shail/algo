def getLCS(strX, strY)
  ...
  lcs = " " * (index + 1)
  # Start from right-most-botom-corner and one by one store characters in lcs
  i, j = strX.length, strY.length
  while i > 0 && j > 0 do
    # if current character in X[] and Y[] are the same, then current character is part of LCS
    if strX[i-1] == strY[j-1]
      lcs[index-1] = strX[i-1]
      i -= 1
      j -= 1
      index -= 1
    # if not then find the larger of hte two and go in the direction of the larger value
