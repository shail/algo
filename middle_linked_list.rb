...
def findMiddleNode(pList)
  slow_pointer = pList
  fast_pointer = pList
  while (!fast_pointer.next.nil?) do
    slow_pointer = slow_pointer.next
    fast_pointer = fast_pointer.next.next.nil? ? fast_pointer.next : fast_pointer.next.next
  end
...
puts findMiddleNode(create_linked_list([1,2])).val.inspect
